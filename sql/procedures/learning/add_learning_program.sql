...
-- Create a procedure to update an existing learning program
...
BEGIN
    ...
    -- Handle any SQL exceptions, rollback transaction, and raise error
    ...
    -- Check if the program exists for the given creator
    IF NOT EXISTS (
    ...
    -- Update the existing program record
    UPDATE dt_learning_programs
    SET 
        title = in_title,
        description = in_description,
        difficulty_level = CASE 
            WHEN in_difficulty_level = 'easy' THEN '0'
    ...
    -- Check for duplicate title by the same creator (excluding current program)
    IF EXISTS (
    ...
    -- Update enrollment expiry date based on updated access period
    ...
    -- Return a JSON response with updated program ID and title
    SELECT JSON_OBJECT(
