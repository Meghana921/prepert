...
BEGIN
 ...
 IF EXISTS( SELECT 1 
  ...
  WHERE creator_tid = in_creator_id AND name = in_template_name) THEN
    SET custom_error = CONCAT( in_template_name, ' - template already exists! You can view and edit template');
  ...
  -- Insert questions using JSON_TABLE (fixed JSON path)
  ...
  FROM (
    SELECT question, COUNT(tid) AS cnt
...
CALL add_eligibility_template(
  3,  -- creator_id (engineering manager's ID)
  'Data Engineer Eligibility 2025',  -- template_name
